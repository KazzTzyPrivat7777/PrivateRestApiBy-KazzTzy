...
export default async function ulzzanggirlHandler(req: Request, res: Response) {
  try {
    const filePath = path.join(__dirname, "ulzzanggirl.json")
    ...
    console.error("Error Ulzzanggirl", error)
    res.status(500).json({
      creator: "KayzzAoshi",
      status: false,
