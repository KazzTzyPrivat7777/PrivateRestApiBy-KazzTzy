...
export default async function aestheticHandler(req: Request, res: Response) {
  try {
    const filePath = path.join(__dirname, "aesthetic.json")
    ...
    console.error("Error Aesthetic", error)
    res.status(500).json({
      creator: "KayzzAoshi",
      status: false,
