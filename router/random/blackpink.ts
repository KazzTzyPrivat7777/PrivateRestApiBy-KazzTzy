...
export default async function blackpinkHandler(req: Request, res: Response) {
  try {
    const filePath = path.join(__dirname, "blackpink.json")
    ...
    console.error("Error Blackpink", error)
    res.status(500).json({
      creator: "KayzzAoshi",
      status: false,
