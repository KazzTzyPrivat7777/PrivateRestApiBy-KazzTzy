...
export default async function cosplayHandler(req: Request, res: Response) {
  try {
    const filePath = path.join(__dirname, "cecanjepang.json")
    ...
    console.error("Error Cosplay", error)
    res.status(500).json({
      creator: "KayzzAoshi",
      status: false,
